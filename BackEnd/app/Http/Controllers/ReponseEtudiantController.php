...
class ReponseEtudiantController extends Controller
{
    ...
    {
        try {
            \Log::info('Bulk submit request:', $request->all());
            
            $validated = $request->validate([
                'tentative_id' => 'required|exists:tentatives,id',
                'responses' => 'required|array|min:1',
                'responses.*.question_id' => 'required|exists:questions,id',
                'responses.*.choix_id' => 'required|exists:choix_reponses,id',
            ]);
            ...
            foreach ($validated['responses'] as $responseData) {
                try {
                    ...
                    $choix = ChoixReponse::find($responseData['choix_id']);

                    if ($question->id !== $choix->question_id) {
                        $errors[] = "Question {$responseData['question_id']}: Answer choice doesn't belong to this question";
                        continue;
                    }

                    $submittedResponse = ReponseEtudiant::create([
                        'tentative_id' => $validated['tentative_id'],
                        'question_id' => $responseData['question_id'],
                        'choix_id' => $responseData['choix_id'],
                        'est_correct' => $choix->est_correct,
                    ]);

                    $submittedResponses[] = $submittedResponse;
                } catch (\Exception $e) {
                    \Log::error('Error submitting response:', ['error' => $e->getMessage(), 'data' => $responseData]);
                    $errors[] = "Question {$responseData['question_id']}: {$e->getMessage()}";
                }
            }
            ...
            return response()->json([
                ...
                'data' => [
                    'submitted' => count($submittedResponses),
                    'responses' => $submittedResponses,
                    'errors' => $errors,
                ],
                'message' => count($submittedResponses) . ' responses submitted' . (count($errors) > 0 ? ' with ' . count($errors) . ' errors' : '')
            ...
            \Log::error('Bulk submit failed:', ['error' => $e->getMessage()]);
            return response()->json([
                'success' => false,
                'message' => 'Failed to submit responses: ' . $e->getMessage()
            ], 500);
        }
    }
